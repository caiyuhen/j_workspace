...
<<<<<<< HEAD
> **注：** API 具备**自动数据类型转换**功能。后端接收请求后，除 `patient_id` 和 `exam_date` 字段外，若发现其他数值特征字段是以带双引号的字符串格式（如 `"120.5"`）传入的，会自动尝试将其解析为纯数值类型（整数或浮点数）。如果遇到无法转换为合法数字的字符串，会拦截请求并返回参数错误提示。
...
当 `records` 中的某个应为数值类型的字段传入了无法解析的字符串（如 `"56a"`）时触发。系统会自动将除 `patient_id` 和 `exam_date` 外的所有带双引号的字符串转换为对应的整数或浮点数，转换失败时将返回该错误。
...
```json
{
  ...
  "message": "字段 'age' 的值 '56a' 无法转换为合法的数字类型。"
}
```
