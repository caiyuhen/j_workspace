...
# -*- coding: utf-8 -*-
"""
最终批量翻译并更新未翻译的 cdisc_synonyms
"""
import pymysql
from final_extended_translator import FinalExtendedTranslator
...
def update_untranslated_batch(translator, batch_size=500):
    """分批更新未翻译的记录"""
    ...
    try:
        ...
        while offset < total_count:
            ...
            conn.commit()
            offset += batch_size
            print(f"✓ 批次提交完成，累计新翻译：{translated_count:,} 条，保留原术语：{unchanged_count:,} 条")
...
def verify_translation():
    ...
    try:
        ...
        cursor.execute("""
            ...
            AND (codelist_name LIKE '%Cardiac%' 
                 OR codelist_name LIKE '%ECG%' 
                 OR codelist_name LIKE '%Holter%'
                 OR codelist_name LIKE '%Dosage%'
                 OR codelist_name LIKE '%Biospecimen%')
            LIMIT 20
        """)
...
if __name__ == '__main__':
    print("="*60)
    print("开始批量翻译未翻译的 cdisc_synonyms（最终扩展）")
    print("="*60)
    
    translator = FinalExtendedTranslator()
    
    # 执行批量翻译
    update_untranslated_batch(translator, batch_size=500)
