...
def add_table(doc, headers, data, title=None):
    ...
    for i, header in enumerate(headers):
        ...
        if shd is None:
            ...
            tcPr.append(shd)
        shd.set(qn('w:fill'), COLORS['accent'].rgb.hex if hasattr(COLORS['accent'].rgb, 'hex') else '93C4F2')
        ...
        if i % 2 == 0:
            for cell in row.cells:
                ...
                if shd is None:
                    shd = OxmlElement('w:shd')
                    tcPr.append(shd)
                shd.set(qn('w:fill'), COLORS['bg_light'].rgb.hex if hasattr(COLORS['bg_light'].rgb, 'hex') else 'F0F5FB')
