...
# 读取原文件
with open('D:/doc/数据培训 PPT.html', 'r', encoding='utf-8') as f:
    content = f.read()

# 创建新的幻灯片 18-22:术语集详解、映射机制、数据质量对比
new_slides = '''
<!-- ══════════ SLIDE 18 : 术语集详解 ══════════ -->
    ...
    <div class="two-col">
      <div>
        ...
        <table class="data-table">
          ...
          <tbody>
            <tr>
              <td><span class="badge badge-blue">EXACT_MATCH</span></td>
              <td>精确匹配，语义完全等价</td>
              <td>ICD-10 J06.9 ↔ SNOMED 199595004</td>
            </tr>
            <tr>
              <td><span class="badge badge-teal">BROAD_MATCH</span></td>
              <td>源概念范围宽于目标</td>
              <td>"感染"涵盖"肺炎"、"尿路感染"</td>
            </tr>
            <tr>
              <td><span class="badge badge-amber">NARROW_MATCH</span></td>
              <td>源概念范围窄于目标</td>
              <td>"链球菌肺炎"是"肺炎"的子集</td>
            </tr>
            <tr>
              <td><span class="badge badge-purple">RELATED_TO</span></td>
              <td>相关但不等价</td>
              <td>"高血压"相关"ACEI 类药物治疗"</td>
            </tr>
          </tbody>
        </table>
        <div class="highlight-box" style="background:rgba(186,117,23,.1);border-color:var(--amber-400);margin-top:14px;">
      ...
      <div>
        ...
        <div class="vocab-mapping">
          <div class="map-row">
            <span class="map-source">ICD-10</span>
            <span class="map-arrow">→</span>
            <span class="map-target">SNOMED-CT</span>
          </div>
          <div class="map-row">
            <span class="map-source">院内代码</span>
            <span class="map-arrow">→</span>
            <span class="map-target">LOINC</span>
          </div>
          <div class="map-row">
            <span class="map-source">药品说明书</span>
            <span class="map-arrow">→</span>
            <span class="map-target">RxNorm</span>
          </div>
          <div class="map-row">
            <span class="map-source">MedDRA</span>
            <span class="map-arrow">→</span>
            <span class="map-target">SNOMED-CT</span>
          </div>
        </div>
  ...
  <div class="slide-inner">
    ...
    <div class="two-col">
      <div>
        ...
        <ul class="rule-list">
          <li>
            <span class="bullet" style="background:rgba(55,138,221,.25);color:#85B7EB;">A</span>
            <div><strong>Attributable</strong> (可归因)<br>每条数据可追溯到原始记录与录入人</div>
          </li>
          <li>
            <span class="bullet" style="background:rgba(55,138,221,.25);color:#85B7EB;">L</span>
            <div><strong>Legible</strong> (清晰可辨)<br>数据格式标准化，无歧义表达</div>
          </li>
          <li>
            <span class="bullet" style="background:rgba(55,138,221,.25);color:#85B7EB;">C</span>
            <div><strong>Contemporaneous</strong> (同步记录)<br>数据采集与试验进程时间同步</div>
          </li>
          <li>
            <span class="bullet" style="background:rgba(55,138,221,.25);color:#85B7EB;">O</span>
            <div><strong>Original</strong> (原始记录)<br>保留数据最初记录形态</div>
          </li>
          <li>
            <span class="bullet" style="background:rgba(55,138,221,.25);color:#85B7EB;">A</span>
            <div><strong>Accurate</strong> (准确无误)<br>通过双重录入、逻辑检核确保准确性</div>
          </li>
        </ul>
        <div class="highlight-box" style="background:rgba(55,138,221,.1);border-color:var(--blue-400);margin-top:14px;">
          <div class="hb-title" style="color:#85B7EB;">数据管理工具链</div>
          <div class="hb-body">① CDISC Validator 自动检查 SDTM/ADaM 合规性 · ② Define.xml 描述数据集结构 · ③ Data Review Initiative (DRI) 人工审核 · ④ Clinical Data Management Plan (CDMP) 流程文档化</div>
        </div>
      </div>
      <div>
    ...
    <div class="two-col">
      <div>
        ...
        <table class="data-table">
          ...
          <tbody>
            <tr>
              <td><span class="badge badge-teal">完整性</span></td>
              <td>NULL 值比例、必填字段缺失率</td>
            </tr>
            <tr>
              <td><span class="badge badge-blue">有效性</span></td>
              <td>概念编码有效性、外键约束</td>
            </tr>
            <tr>
              <td><span class="badge badge-amber">一致性</span></td>
              <td>性别与出生性别一致性</td>
            </tr>
            <tr>
              <td><span class="badge badge-purple">及时性</span></td>
              <td>数据更新延迟天数</td>
            </tr>
            <tr>
              <td><span class="badge badge-teal">准确性</span></td>
              <td>逻辑矛盾检测 (死亡日期 < 出生日期)</td>
            </tr>
          </tbody>
        </table>
        <div class="highlight-box" style="background:rgba(29,158,117,.1);border-color:var(--teal-400);margin-top:14px;">
      ...
      <div>
        ...
        <ul class="rule-list">
          <li>
            <span class="bullet" style="background:rgba(29,158,117,.25);color:#5DCAA5;">✓</span>
            <div><strong>Concept Validity</strong><br>所有 concept_id 必须在 CONCEPT 表中存在</div>
          </li>
          <li>
            <span class="bullet" style="background:rgba(29,158,117,.25);color:#5DCAA5;">✓</span>
            <div><strong>Visit Consistency</strong><br>visit_end_date ≥ visit_start_date</div>
          </li>
          <li>
            <span class="bullet" style="background:rgba(29,158,117,.25);color:#5DCAA5;">✓</span>
            <div><strong>Gender Consistency</strong><br>gender_concept_id 必须为 8507 (男) 或 8532 (女)</div>
          </li>
          <li>
            <span class="bullet" style="background:rgba(29,158,117,.25);color:#5DCAA5;">✓</span>
            <div><strong>Age Plausibility</strong><br>age_at_visit 应在 0-120 岁范围内</div>
          </li>
          <li>
            <span class="bullet" style="background:rgba(29,158,117,.25);color:#5DCAA5;">✓</span>
            <div><strong>Drug Dosage Validity</strong><br>dosage_unit_concept_id 必须为标准单位</div>
          </li>
        </ul>
        <div class="highlight-box" style="background:rgba(29,158,117,.1);border-color:var(--teal-400);margin-top:14px;">
    ...
    <table class="data-table" style="margin-bottom:16px;">
      ...
      <tbody>
        <tr>
          ...
          <td>监管申报合规，确保数据可追溯</td>
          <td>观察性研究可重复，数据可横向对比</td>
        </tr>
        <tr>
          <td><strong>核心方法</strong></td>
          <td>ALCOA+ 原则 + 人工审核</td>
          <td>DQD 自动检核 + ACHILLES 特征分析</td>
        </tr>
        <tr>
          <td><strong>检核规则</strong></td>
          <td>CDISC Validator 预定义规则</td>
          <td>360+ DQD 可配置规则</td>
        </tr>
        <tr>
          <td><strong>问题处理</strong></td>
          <td>数据质疑 (Query)→修复→重新验证</td>
          <td>质量报告→数据源反馈→ETL 修正</td>
        </tr>
        <tr>
          <td><strong>文档化要求</strong></td>
          <td>Define.xml + 数据管理计划 (CDMP)</td>
          <td>ETL 文档 + ACHILLES 报告</td>
        </tr>
        <tr>
          <td><strong>适用场景</strong></td>
          <td>临床试验 (RCT) 数据申报</td>
          <td>真实世界 (RWE) 多中心研究</td>
        </tr>
        <tr>
        ...
        <tr>
          <td><strong>技术工具</strong></td>
          <td>SAS、R、CDISC Validator</td>
          <td>ATLAS、DQD、ACHILLES、WhiteRabbit</td>
        </tr>
      </tbody>
    </table>
    <div class="highlight-box" style="background:rgba(186,117,23,.1);border-color:var(--amber-400);">
...
# 查找 s18 的位置，在其之前插入新幻灯片
insert_pos = content.find('<!-- ══════════ SLIDE 18 : CDISC vs OMOP ══════════ -->')
if insert_pos != -1:
    # 替换 slide 编号，将原 18-19 改为 23-24
    content_after = content[insert_pos:]
    content_after = re.sub(r'id="s18"', 'id="s23"', content_after)
    content_after = re.sub(r'id="s19"', 'id="s24"', content_after)
    content = content[:insert_pos] + new_slides + content_after
    
    # 更新总结 slide 内容
    content = re.sub(
        r'③ CDISC.*?④ OMOP CDM',
        '③ CDISC · ④ OMOP CDM · ⑤ 术语集详解 (MedDRA/LOINC/RxNorm/SNOMED) · ⑥ 数据质量对比 (CDISC 管理 vs OMOP 评估)',
        content, 
        flags=re.DOTALL
    )
    
    # 更新计数器
    content = re.sub(r'>1 / 20<', '>1 / 25<', content)
else:
    print("Warning: Could not find slide 18 marker")

# 写入新文件
with open('D:/doc/数据培训 PPT_完整版_v2.html', 'w', encoding='utf-8') as f:
    f.write(content)

print("✅ 增强版 PPT 已生成：数据培训 PPT_完整版_v2.html")
print("新增内容:")
