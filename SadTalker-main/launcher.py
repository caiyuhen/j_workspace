...
def run_pip(args, desc=None):
    ...
    # Temporarily unset proxies if there are any issues with downloading
    os.environ['HTTP_PROXY'] = ''
    os.environ['HTTPS_PROXY'] = ''
    os.environ['ALL_PROXY'] = ''
    return run(f'"{python}" -m pip {args} --prefer-binary{index_url_line} -i https://pypi.org/simple', desc=f"Installing {desc}", errdesc=f"Couldn't install {desc}")
...
def prepare_environment():
    ...
    run_pip(f"install -r \"{requirements_file}\" -i https://pypi.org/simple", "requirements for SadTalker WebUI (may take longer time in first time)")
...
if __name__ == "__main__":
    # 在 Docker 中，环境已经在构建镜像时配置好，不需要再次检查和安装，否则可能会卡住或报错
    # prepare_environment()
    start()