...
const getSourceCatalog = vi.fn(async () => ({
    available_sources: sourceConfigResponse.available_sources.map(({ enabled, ...item }) => item),
...
vi.mock("@meda/shared-sdk", () => {
    ...
    const build_grouped_terms_from_pico = (pico) => {
        const groups = { p: [], i: [], c: [], o: [] };
        for (const key of ["p", "i", "c", "o"]) {
            const raw = pico[key] || "";
            const terms = raw.split(/[\s,;，；/]+/).filter((t) => t.length > 0);
            groups[key] = terms.map((label, idx) => ({
    ...
    const ensureDemoProjectAndQuery = vi.fn(({ workspaceHomeProjectId }) => ({
    ...
    return {
        ...
        createClient: () => ({
            ...
            getMe: vi.fn(),
            getSearchRun: vi.fn(),
            createProject: vi.fn((payload) => ({ id: 99, name: payload.name, workspace_key: payload.org_slug + "/" + payload.name })),
            getSearchRunCsvUrl: (_, pid, rid) => `/api/projects/${pid}/search-runs/${rid}/csv`,
...
test("web workspace opens a stage-entry hub from the stage card", async () => {
    ...
    expect(screen.getByRole("button", { name: "运行当前检索", hidden: true })).toBeInTheDocument();
});
test("web workspace opens query builder and creates a version", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "检索式管理" })).toBeInTheDocument();
...
test("web workspace opens source config and saves an extra database", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "数据库来源" })).toBeInTheDocument();
    ...
    expect(await screen.findByText("已启用 2 个数据库：PubMed, Cochrane Library")).toBeInTheDocument();
});
test("web workspace edits search fields, year range and languages", async () => {
    ...
    expect(await screen.findByRole("checkbox", { name: "主题词" })).not.toBeChecked();
...
test("web workspace imports literature and confirms a duplicate", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "文献条目库" })).toBeInTheDocument();
    expect(screen.getByText("共 2 条 · 唯一 1 条 · 重复 1 条")).toBeInTheDocument();
    expect(screen.getByText("Metformin and cardiovascular outcomes")).toBeInTheDocument();
    ...
    expect(await screen.findByText("本次导入 2 条 · 重复 1 条 · 跳过 0 条")).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "标记为独立文献" }));
    expect(confirmLiteratureUnique).toHaveBeenCalledWith(1, 12);
    expect(await screen.findByText("共 2 条 · 唯一 2 条 · 重复 0 条")).toBeInTheDocument();
});
