...
class PPGSleepAnalyzer:
    ...
    def analyze_sleep_apnea(self, ppg_data: np.ndarray, timestamps: List[str]) -> Dict[str, Any]:
        """
        睡眠呼吸暂停分析
    ...
    def analyze_nocturnal_spo2(self, ppg_data: np.ndarray, timestamps: List[str]) -> Dict[str, Any]:
        """
        夜间血氧饱和度分析
    ...
    def analyze_blood_pressure_rhythm(self, ppg_data: np.ndarray, timestamps: List[str]) -> Dict[str, Any]:
        """
        血压节律分析
    ...
    def _extract_respiratory_signal(self, ppg_data: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            # 如果滤波失败，返回简化的呼吸信号
            respiratory_signal = signal.savgol_filter(ppg_data, min(len(ppg_data)//10, 51), 3)
        
        return respiratory_signal
    
    def _detect_apnea_events(self, respiratory_signal: np.ndarray, timestamps: List[str]) -> List[Dict]:
        """检测呼吸暂停事件"""
        events = []
        ...
        for start, end in zip(starts, ends):
            ...
            if duration >= self.apnea_duration_min:
                events.append({
                    'start_time': timestamps[min(start, len(timestamps)-1)] if start < len(timestamps) else timestamps[-1],
                    'end_time': timestamps[min(end-1, len(timestamps)-1)] if end <= len(timestamps) else timestamps[-1],
    ...
    def _detect_hypopnea_events(self, respiratory_signal: np.ndarray, timestamps: List[str]) -> List[Dict]:
        """检测低通气事件"""
        events = []
    ...
    def _estimate_spo2(self, ppg_data: np.ndarray) -> np.ndarray:
        ...
        try:
            # 计算AC成分（交流成分）
            ac_component = ppg_data - signal.savgol_filter(ppg_data, 
                                                          min(len(ppg_data)//10, 51), 3)
            
            # 计算DC成分（直流成分）
            dc_component = signal.savgol_filter(ppg_data, 
                                              min(len(ppg_data)//10, 51), 3)
    ...
    def _identify_night_period(self, timestamps: List[str]) -> np.ndarray:
        """识别夜间时段"""
        night_indices = []
        ...
        for i, timestamp_str in enumerate(timestamps):
            try:
                ...
                # 判断是否为夜间时段
                if hour >= self.night_hours[0] or hour < self.night_hours[1]:
                    night_indices.append(i)
                    
            except (ValueError, TypeError):
                continue
        
        return np.array(night_indices)
    
    def _detect_desaturation_events(self, spo2_values: np.ndarray, timestamps: List[str], 
                                  night_indices: np.ndarray) -> List[Dict]:
        """检测血氧饱和度下降事件"""
        events = []
        
        # 检测SpO2下降超过3%的事件
        baseline = signal.savgol_filter(spo2_values, min(len(spo2_values)//10, 51), 3)
        ...
        for start, end in zip(starts, ends):
            ...
            if duration >= 10:  # 至少持续10秒
                ...
                events.append({
                    'start_time': timestamps[min(actual_start, len(timestamps)-1)],
                    'end_time': timestamps[min(actual_end, len(timestamps)-1)],
                    'duration_seconds': round(duration, 1),
    ...
    def _analyze_circadian_rhythm(self, bp_indicators: np.ndarray, timestamps: List[str]) -> Dict[str, Any]:
        """分析昼夜节律"""
        # 按小时分组计算平均值
        hourly_bp = {}
        ...
        for i, timestamp_str in enumerate(timestamps):
            ...
            try:
                ...
                if hour not in hourly_bp:
                    hourly_bp[hour] = []
                hourly_bp[hour].append(bp_indicators[i])
        ...
        # 计算每小时平均值
        hourly_means = {}
        for hour, values in hourly_bp.items():
    ...
    def _analyze_nocturnal_dipping(self, bp_indicators: np.ndarray, timestamps: List[str]) -> Dict[str, Any]:
        """分析夜间血压下降"""
        # 识别白天和夜间时段
        day_values = []
        night_values = []
    ...
    def _analyze_bp_variability(self, bp_indicators: np.ndarray, timestamps: List[str]) -> Dict[str, float]:
        """分析血压变异性"""
        # 短期变异性（相邻测量值差异）
    ...
    def _assess_rhythm_quality(self, bp_indicators: np.ndarray, timestamps: List[str]) -> str:
        ...
        data_continuity = len(bp_indicators) / len(timestamps) if timestamps else 0
...
def analyze_sleep_data(ppg_data: np.ndarray, timestamps: List[str], 
