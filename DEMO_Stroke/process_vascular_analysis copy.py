...
class VascularAnalysisProcessor:
    ...
    def save_analysis_result(self, device_id, analysis_result):
        ...
        try:
            ...
            if _orjson is not None:
                # 使用 orjson 进行更快的序列化，并开启对 numpy 与非字符串键的支持
                options = _orjson.OPT_INDENT_2 | _orjson.OPT_SERIALIZE_NUMPY | _orjson.OPT_NON_STR_KEYS
                with open(output_path, 'wb') as f:
                    f.write(_orjson.dumps(serializable_result, option=options))
            else:
    ...
    def analyze_inflammation_segment(self, decompressed_data, segment_index, segment_info):
        ...
        """
        try:
            ppg_signal = np.array(decompressed_data, dtype=np.float64)
    ...
    def process_device_data(self, file_path):
        ...
        for i, segment in enumerate(processed_data):
            try:
                ...
                # 血管功能分析
                vascular_result = self.analyzer.analyze_ppg_segment(decompressed_data)
                ...
                # 血流分析
                blood_flow_result = self.analyze_blood_flow_segment(decompressed_data, i, segment)
                if blood_flow_result is not None:
                    blood_flow_results.append(blood_flow_result)
                
                # 心律不齐分析
                arrhythmia_result = self.analyze_arrhythmia_segment(decompressed_data, i, segment)
                if arrhythmia_result is not None:
                    arrhythmia_results.append(arrhythmia_result)
                
                # 炎症分析
                inflammation_result = self.analyze_inflammation_segment(decompressed_data, i, segment)
                if inflammation_result is not None:
                    inflammation_results.append(inflammation_result)
                
                
                # 收集数据用于睡眠分析
                all_ppg_data.extend(decompressed_data)
                collect_time = segment.get('collectTime')
                if collect_time:
                    # 为每个数据点生成时间戳（假设125Hz采样率）
                    base_time = datetime.strptime(collect_time, "%Y-%m-%d %H:%M:%S")
                    all_timestamps.extend([
                        (base_time + timedelta(seconds=j/125.0)).strftime("%Y-%m-%d %H:%M:%S")
        ...
        if all_ppg_data and all_timestamps:
            try:
                print(f"开始睡眠分析，数据点数量: {len(all_ppg_data)}")
                sleep_analysis_result = analyze_sleep_data(
                    np.array(all_ppg_data), 
                    all_timestamps, 
                    sampling_rate=125
                )
                print("睡眠分析完成")
    ...
    def analyze_arrhythmia_segment(self, decompressed_data, segment_index, segment_info):
        ...
        """
        try:
            ppg_signal = np.array(decompressed_data, dtype=float)
    ...
    def process_all_files(self):
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            futures = [executor.submit(_process_and_save_entry, fp, self.input_dir, self.output_dir) for fp in file_paths]
            total = len(futures)
