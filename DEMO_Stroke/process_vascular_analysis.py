    ...
    def process_device_data(self, file_path):
        ...
        for i, segment in enumerate(processed_data):
            try:
                ...
                # 血管功能分析
                ppg_signal = np.asarray(decompressed_data, dtype=float)
                vascular_result = self.analyzer.analyze_ppg_segment(ppg_signal)
                ...
                if collect_time:
                    ...
                    strings = np.char.replace(np.datetime_as_string(times, unit='s'), 'T', ' ')
                    all_timestamps.extend(strings.tolist())
