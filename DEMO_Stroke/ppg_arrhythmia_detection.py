...
class PPGArrhythmiaDetector:
    ...
    def _calculate_signal_quality(self, signal_array):
        ...
        try:
            ...
            periodicity = np.max(autocorr[self.min_peak_distance:self.max_peak_distance]) / autocorr[0]
            
            # 综合SQI评分
            sqi = (np.clip(snr / 20, 0, 1) * 0.4 + 
                   np.clip(baseline_stability, 0, 1) * 0.3 + 
                   np.clip(periodicity, 0, 1) * 0.3)
