...
class CardiovascularRiskCalculator:
    ...
    def __init__(self, use_dynamic_thresholds=True, validation_mode=False):
        ...
        self.fixed_thresholds = {
            'low_risk': 0.25,   # 低风险上限 (调整为0.40)
            'medium_risk': 0.35,  # 中风险上限 (调整为0.50)
            'high_risk': 0.35   # 高风险下限 (调整为0.60)
        }
    ...
    def generate_risk_report(self, file_path: str) -> Dict[str, Any]:
        ...
        # 核心风险计算
        mi_risk_score = self.calculate_mi_risk_score(cleaned_data)
        stroke_risk_score = self.calculate_stroke_risk_score(cleaned_data)
        risk_results = {
            'mi_risk_score': mi_risk_score,
            'stroke_risk_score': stroke_risk_score
        }
        ...
        risk_results.update(risk_ratios)
        ...
        # 注入时间字段（从分析结果读取）
        try:
            source_time_info = data.get('source_time_info', {}) or {}
            collect_time = (
                source_time_info.get('collectTime_first')
                or data.get('collect_time')
                or data.get('collectTime')
            )
            create_time = (
                source_time_info.get('createTime_first')
                or data.get('create_time')
                or data.get('createTime')
            )
            collect_time_beijing = (
                source_time_info.get('collectTime_beijing_first')
                or data.get('collect_time_beijing')
                or data.get('collectTime_beijing')
            )
            create_time_beijing = (
                source_time_info.get('createTime_beijing_first')
                or data.get('create_time_beijing')
                or data.get('createTime_beijing')
            )

            report['collectTime'] = collect_time
            report['createTime'] = create_time
            report['collectTime_beijing'] = collect_time_beijing
            report['createTime_beijing'] = create_time_beijing
        except Exception:
            # 保持稳健：如果时间注入过程异常，不影响报告生成
            pass
    ...
    def validate_and_clean_data(self, data):
        ...
        if 'vascular_function_statistics' in data and 'heart_rate_stats' in data['vascular_function_statistics']:
            ...
            if lf_hf_ratio < 0.1 or lf_hf_ratio > 10:  # LF/HF正常范围通常在0.5-3.0
                ...
                if hr_std > 0:
                    # 根据心率变异性估算LF/HF比值
                    cv = hr_std / hr_mean if hr_mean > 0 else 0 # 变异系数
                    ...
                    cleaned_data['vascular_function_statistics']['heart_rate_stats']['lf_hf_ratio_mean'] = 1.0
...
def parse_arguments():
    ...
    parser.add_argument('-o', '--output', 
                       default='./risk_assessment_results_2',
                       help='输出结果目录路径（支持相对路径，默认: ./risk_assessment_results_2）')
