...
#### 天气数据获取
```bash
curl http://localhost:5008/api/weather
```

#### 空气质量数据
```bash
curl http://localhost:5008/api/weather/air-quality
```
...
#### 训练新模型
```bash
python main.py --model_type gradient_boosting --use_smote --calibrate
```
...
#### 批量预测
```bash
python main.py --data_path data/raw/patient_data.csv --predict_only
```
